...
|---|---|---|---|---|---|---|
| ardupilot_manager | mavlink_router_crash | `stop_autopilot` / NP-77 (as generator) / `tools/fault_injection.sh` | probed | `tools/fault_injection.sh --kill-process mavlink-routerd --freshness-url <HEARTBEAT> --watch "<mavlink> <mcm streams>" --restore-post <.../v1.0/restart` | 88-navigator | B6/B7. Exercised via process kill, not `tmux kill-session`: the `autopilot` session also holds `ardupilot_manager` itself, so killing it would kill the supervisor whose recovery is the behaviour under test. Result in `FINDINGS.md`: the silent-200 expectation held and `auto_restart_router` did **not** recover. |
...
| commander | settings_reset_partial_failure | `reset_blueos_settings` | probed | confirmed `POST /commander/v1.0/settings/reset?i_know_what_i_am_doing=true`, then reboot so the defaults are re-applied from scratch, then sweep for leftovers | 88-navigator | No partial failure: every service returned 200 afterwards, and the `main.py:193-199` ignore list held exactly — `ardupilot-manager` kept its 6 endpoints, `kraken` kept `major_tom`, firmware stayed 4.7.0. **`cable-guy` is not in that list**, so the reset does discard the network config; `.0.88` only survived because the same MAC reclaimed its DHCP lease (F-094). Reaching this mode needs `sacrificial`, and it should also be gated by `never_strand_mgmt`. |
