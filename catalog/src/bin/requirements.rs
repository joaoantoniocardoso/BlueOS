...
use blueos_catalog::requirements_report::{
    ...
    render_srs, requirements_json, validate_rtm_completeness,
};
...
fn main() -> ExitCode {
    ...
    if !json_output && !srs_output && !rtm_output && diff_tag.is_none() {
        eprintln!("error: specify at least one of --json, --srs, --rtm, --diff");
        print_help();
...
fn print_help() {
    eprintln!(
        "usage:\n\
         \x20 requirements --version <tag> [--json] [--srs] [--rtm] [--diff <tag>]\n\
         \n\
