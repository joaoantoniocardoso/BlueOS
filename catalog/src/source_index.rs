...
pub fn catalog_module_for_entity(entity: &CatalogEntity, citation: &Citation) -> String {
    ...
    match entity {
        ...
        CatalogEntity::Journey { id } => crate::journeys::source_file_for_journey(*id)
...
fn bootstrap_entity(citation: &Citation) -> CatalogEntity {
    ...
    match segs.as_slice() {
        ["services", idx, ..] => {
            ...
            match crate::services::SERVICES.get(index) {
        ...
        ["pages", idx, ..] => {
            ...
            match crate::pages::PAGES.get(index) {
        ...
        ["journeys", idx, ..] => {
            ...
            match crate::journeys::all_journeys().get(index) {
...
mod tests {
    ...
    fn bootstrap_slices_match_attribution_index_sources() {
        ...
        assert_eq!(catalog.services().len(), crate::services::SERVICES.len());
        for (idx, service) in catalog.services().iter().enumerate() {
            assert_eq!(
                service.id,
                crate::services::SERVICES[idx].id,
                "service index {idx} diverged from SERVICES slice"
            );
        }

        let journeys = crate::journeys::all_journeys();
        ...
        assert_eq!(catalog.pages().len(), crate::pages::PAGES.len());
        for (idx, page) in catalog.pages().iter().enumerate() {
            assert_eq!(
                page.id,
                crate::pages::PAGES[idx].id,
