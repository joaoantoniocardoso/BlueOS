...
use crate::harness_ratchet::{DEFAULT_BASELINE_PATH, FAILURE_MODE_LEDGER_PATH};
...
pub fn repo_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("catalog crate has a parent directory")
        .to_path_buf()
}
...
pub fn catalog_site_for_citation(citation: &Citation) -> Option<PathBuf> {
    ...
    match segs.as_slice() {
        ["services", idx, ..] => {
            ...
            let id = crate::services::SERVICES.get(index)?.id;
        ...
        ["pages", idx, ..] => {
            let index: usize = idx.parse().ok()?;
            let name = crate::pages::PAGES.get(index)?.id.as_str();
            Some(PathBuf::from(format!("pages/{name}.rs")))
        }
        ["journeys", idx, ..] => {
            let index: usize = idx.parse().ok()?;
            let id = crate::journeys::all_journeys().get(index)?.id;
            crate::journeys::source_file_for_journey(id).map(PathBuf::from)
        }
        _ => None,
    }
}

fn service_mod_name(id: crate::id::ServiceId) -> &'static str {
    match id {
        crate::id::ServiceId::MavlinkCameraManager => "mavlink_camera_manager",
        other => other.as_str(),
    }
}
...
mod tests {
    use super::*;
    use crate::journey::BLAST_RADIUS_UNKNOWN;
    use crate::provenance::{Evidence, Grounded, Observed, Provenance};
    ...
    fn source_to_asserted_downgrade_trips_citation_pins() {
        const FILE_CITATION_COUNT: usize = 2041;
    ...
    fn mark_applied_relocations_clears_relocated_so_gate_passes() {
        let pardal_idx = crate::services::SERVICES
            .iter()
            .position(|service| service.id == crate::id::ServiceId::Pardal)
    ...
    fn mark_applied_relocations_distinguishes_wifi_service_and_journey_sites() {
        let wifi_service_idx = crate::services::SERVICES
            .iter()
            .position(|service| service.id == crate::id::ServiceId::Wifi)
            .expect("wifi service");
        let wifi_journey_idx = crate::journeys::all_journeys()
            .iter()
            .position(|journey| {
                crate::journeys::source_file_for_journey(journey.id) == Some("journeys/wifi.rs")
            })
            .expect("wifi journey");
