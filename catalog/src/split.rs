...
pub(crate) fn modularity_partition(n: usize, weights: &[Vec<f64>]) -> (Vec<Vec<usize>>, f64) {
    if n == 0 {
        return (Vec::new(), 0.0);
    }
...
pub(crate) fn consensus_over(partitions: &[Vec<Vec<usize>>], n: usize) -> ConsensusIdx {
...
pub(crate) fn group_by_key<K: Ord + Clone>(keys: &[K]) -> Vec<Vec<usize>> {
...
pub(crate) fn clique_weights(n: usize, sets: &[Vec<usize>]) -> Vec<Vec<f64>> {
