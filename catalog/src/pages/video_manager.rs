...
pub fn page() -> Page {
    Page {
        ...
        component: Observed::known(
            "core/frontend/src/views/VideoManagerView.vue".to_string(),
            Evidence {
                file: "core/frontend/src/router/index.ts".to_string(),
                line: 64,
            },
        ...
        stores: ObservedSet::known(vec![
            Evidenced::new(
                "video".to_string(),
                Evidence {
                    file: "core/frontend/src/components/video-manager/VideoManager.vue".to_string(),
                    line: 92,
                },
            ),
            Evidenced::new(
            ...
            Evidenced::new(
                "beacon".to_string(),
                Evidence {
                    file: "core/frontend/src/components/video-manager/VideoDiagnosticHelper.vue".to_string(),
                    line: 29,
                },
            ),
        ]),
        consumes: ObservedSet::known(vec![
            Evidenced::new(
                ...
                Evidence {
                    file: "core/frontend/src/store/video.ts".to_string(),
                    line: 144,
                },
            ),
            Evidenced::new(
            ...
            Evidenced::new(
                ...
                Evidence {
                    file: "core/frontend/src/components/video-manager/VideoControlsDialog.vue".to_string(),
                    line: 190,
                },
            ),
            Evidenced::new(
            ...
            Evidenced::new(
                ...
                Evidence {
                    file: "core/frontend/src/components/video-manager/VideoStream.vue".to_string(),
                    line: 311,
                },
            ),
            Evidenced::new(
                ...
                Evidence {
                    file: "core/frontend/src/store/commander.ts".to_string(),
                    line: 115,
                },
            ),
            Evidenced::new(
        ...
        client_state: AssertedSet::established(vec![
            Rationaled::new(
                ClientState {
                    name: "settings dialog state".to_string(),
                    store: "VideoManager.vue component data".to_string(),
                    ownership: StateOwnership::FrontendOwned,
                    notes: "show_settings_dialog and legacy_mode toggle before commander sync".to_string(),
                },
                "gear-button settings dialog and legacy camera switch are ephemeral UI state",
            ),
            Rationaled::new(
                ClientState {
                    name: "stream creation/editing form".to_string(),
                    store: "VideoStreamCreationDialog.vue component data".to_string(),
                    ownership: StateOwnership::FrontendOwned,
                    notes: "stream_name, selected_encode/size/interval, stream_endpoints, extended_configuration checkboxes".to_string(),
                },
                "stream configuration wizard holds form fields locally until POST /streams",
            ),
            Rationaled::new(
                ClientState {
                    name: "device dialog visibility".to_string(),
                    store: "VideoDevice.vue component data".to_string(),
                    ownership: StateOwnership::FrontendOwned,
                    notes: "show_controls_dialog and show_stream_creation_dialog flags".to_string(),
                },
                "per-device dialog open/close state is not persisted",
            ),
            Rationaled::new(
                ClientState {
                    name: "stream card UI state".to_string(),
                    store: "VideoStream.vue component data".to_string(),
                    ownership: StateOwnership::FrontendOwned,
                    notes: "show_stream_edit_dialog and isExpanded error-text expansion".to_string(),
                },
                "stream card edit dialog and error expand/collapse are local UI state",
            ),
            Rationaled::new(
                ClientState {
                    name: "thumbnail preview mode".to_string(),
                    store: "VideoThumbnail.vue component data".to_string(),
                    ownership: StateOwnership::FrontendOwned,
                    notes: "continuous_mode, snapshot_in_progress, snapshot_cooldown, last_fetch_ms, local thumbnail ref".to_string(),
                },
                "thumbnail component tracks preview mode and timing independent of backend",
            ),
            Rationaled::new(
                ClientState {
                    name: "device control update guard".to_string(),
                    store: "VideoControlsDialog.vue component data".to_string(),
                    ownership: StateOwnership::FrontendOwned,
                    notes: "updating flag disables controls while POST /v4l batch is in flight".to_string(),
                },
                "controls dialog tracks in-flight submission state locally",
            ),
            Rationaled::new(
                ClientState {
                    name: "video devices cache".to_string(),
                    store: "store/video".to_string(),
                    ownership: StateOwnership::BackendOwned,
                    notes: "available_devices mirrored from GET /v4l".to_string(),
                },
                "device list is fetched from mavlink-camera-manager and displayed without client mutation",
            ),
            Rationaled::new(
                ClientState {
                    name: "video streams cache".to_string(),
                    store: "store/video".to_string(),
                    ownership: StateOwnership::BackendOwned,
                    notes: "available_streams mirrored from GET /streams".to_string(),
                },
                "stream list and per-stream status/error fields come from backend polling",
            ),
            Rationaled::new(
                ClientState {
                    name: "fetch loading and error flags".to_string(),
                    store: "store/video".to_string(),
                    ownership: StateOwnership::Shared,
                    notes: "updating_devices, updating_streams, fetch_devices_error, fetch_streams_error".to_string(),
                },
                "loading/error flags are client-managed around backend fetch lifecycle",
            ),
            Rationaled::new(
                ClientState {
                    name: "thumbnail blob cache".to_string(),
                    store: "store/video thumbnails Map".to_string(),
                    ownership: StateOwnership::Shared,
                    notes: "blob URLs created client-side from GET /thumbnail responses; revoked on refresh".to_string(),
                },
                "thumbnails are fetched from backend but cached and lifecycle-managed in the browser",
            ),
            Rationaled::new(
                ClientState {
                    name: "device control values".to_string(),
                    store: "device.controls in VideoControlsDialog.vue".to_string(),
                    ownership: StateOwnership::Shared,
                    notes: "control Slider/Menu/Bool values from v4l response, mutated locally then POSTed".to_string(),
                },
                "V4L controls are mirrored from backend device payload then edited client-side before submit",
            ),
            Rationaled::new(
                ClientState {
                    name: "filtered device list".to_string(),
                    store: "VideoManager.vue computed video_devices".to_string(),
                    ownership: StateOwnership::Shared,
                    notes: "filters H264 support, Fake/RadCam/Redirect rules, sorts by name".to_string(),
                },
                "displayable devices are derived client-side from backend device and stream caches",
            ),
            Rationaled::new(
                ClientState {
                    name: "per-device stream associations".to_string(),
                    store: "VideoDevice.vue computed device_streams via utils/video".to_string(),
                    ownership: StateOwnership::Shared,
                    notes: "available_streams_from_device matches streams to device source paths".to_string(),
                },
                "streams per device card are client-derived from the global streams cache",
            ),
            Rationaled::new(
                ClientState {
                    name: "beacon IP addresses".to_string(),
                    store: "store/beacon".to_string(),
                    ownership: StateOwnership::BackendOwned,
                    notes: "client_ip_address and nginx_ip_address read for endpoint defaults and diagnostics; beacon polls globally in background".to_string(),
                },
