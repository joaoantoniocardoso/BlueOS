//! Regenerates `catalog/crates/catalog-data/src/journey_presence.rs` and `catalog/feature_presence_map.json`
...
use crate::shell::{repo_root, run as run_git, run_ok};
...
pub fn run() -> Result<(), String> {
    let root = repo_root();
    let out_rs = root.join("catalog/crates/catalog-data/src/journey_presence.rs");
    ...
    let mut lines = vec![
        ...
        String::new(),
        "use catalog_kernel::version::Availability;".to_string(),
        String::new(),
        format!(
