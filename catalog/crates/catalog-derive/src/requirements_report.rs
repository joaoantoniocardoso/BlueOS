...
pub fn overlay_trace_failures() -> Vec<String> {
    ...
    let repo = match tracked_paths(&repo_root()) {
        Ok(paths) => paths,
        Err(err) => {
            failures.push(err);
            return failures;
...
fn journey_resolvable_evidence(
    catalog: &Catalog,
    journey_id: catalog_kernel::id::journey::JourneyId,
) -> String {
...
fn capture_resolves(file: &str, key: &str) -> bool {
    let path = catalog_dir().join(file);
...
mod tests {
    ...
    const FILTERED_COUNT_1_4_0: usize = 379;
