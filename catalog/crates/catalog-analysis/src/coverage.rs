...
use crate::coverage_mappings::TRACKER_MAPPINGS;
...
pub fn coverage_report(catalog: &Catalog) -> CoverageReport {
    let journey_index: HashMap<JourneyId, &catalog_model::journey::UseCase> = catalog
        .journeys()
        .iter()
...
mod tests {
    ...
    fn every_csv_tracker_task_is_mapped() {
        let csv_path = catalog_paths::catalog_dir().join(TRACKER_CSV_PATH);
