...
use catalog_kernel::provenance::{Asserted, AssertedSet, Grounded, GroundedSet, ObservedSet};
use catalog_model::edge::{Bus, Connection, FailureImpact};
use catalog_model::page::ConsumeTarget;
use catalog_model::resource::ResourceOwnership;
use catalog_model::service::ServiceJudgment;
...
pub trait CatalogClustering {
    fn coupling_matrix(&self, policy: ClusterPolicy) -> CouplingMatrix;
    fn cluster(&self, policy: ClusterPolicy) -> ClusterResult;
    fn cluster_stability(&self, policy: ClusterPolicy, runs: usize, jitter: f64)
        -> StabilityReport;
    fn boundary_proposals(&self) -> Vec<ClusterResult>;
    fn page_cooccurrence_matrix(&self) -> CouplingMatrix;
    fn journey_cooccurrence_matrix(&self) -> CouplingMatrix;
    fn split_lenses(&self) -> Vec<LensPartition>;
    fn split_consensus(&self) -> SplitConsensus;
}

impl CatalogClustering for catalog_core::catalog::Catalog {
    fn coupling_matrix(&self, policy: ClusterPolicy) -> CouplingMatrix {
...
pub fn greedy_modularity_communities(n: usize, weights: &[Vec<f64>]) -> Vec<Vec<usize>> {
    if n == 0 {
        return Vec::new();
    }
...
pub fn modularity_q_indices(weights: &[Vec<f64>], communities: &[Vec<usize>]) -> f64 {
    let mass = total_edge_mass(weights);
    if mass == 0.0 {
        return 0.0;
    }
...
pub fn bus_label(bus: Bus) -> &'static str {
    match bus {
        Bus::Rest => "rest",
        Bus::Zenoh => "zenoh",
        Bus::Mavlink => "mavlink",
...
mod tests {
    use super::*;
    use crate::cluster::CatalogClustering;
    use catalog_core::catalog::Catalog;
