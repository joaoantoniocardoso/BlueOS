...
use catalog_kernel::provenance::{Grounded, GroundedSet, ObservedSet};
use catalog_model::journey::{HttpMethod, RouteRef, UseCase};
use catalog_model::page::{ConsumeTarget, Page};
...
fn coverage_hits(catalog: &Catalog) -> BTreeMap<ApiContractKey, BTreeSet<ApiCoverageSource>> {
    ...
    for probe in catalog_harness::negative_probes::NEGATIVE_PROBES {
...
fn insert_slo_hits(
    catalog: &Catalog,
    baselines: &GroundedSet<catalog_model::runtime::SloBaseline>,
    hits: &mut BTreeMap<ApiContractKey, BTreeSet<ApiCoverageSource>>,
) {
...
fn insert_probe_hit(
    catalog: &Catalog,
    probe: &catalog_harness::negative_probes::NegativeProbe,
    hits: &mut BTreeMap<ApiContractKey, BTreeSet<ApiCoverageSource>>,
) {
...
fn insert_route_hit(
    ...
    let (version, path) = if let Some(resolved) =
        catalog_core::http::resolve_http_path(catalog, route)
    {
