...
pub trait CatalogJourneyGrouping {
    fn journey_split_consensus(&self) -> JourneySplitConsensus;
}

impl CatalogJourneyGrouping for catalog_core::catalog::Catalog {
    fn journey_split_consensus(&self) -> JourneySplitConsensus {
        ...
        let service_weights = shared_service_weights(self, n);
        let (service_groups, service_q) = modularity_partition(n, &service_weights);

        let capability_weights = shared_capability_weights(self, n);
        let (capability_groups, capability_q) = modularity_partition(n, &capability_weights);

        let chain_groups = chain_components(self, n, &index);
...
fn shared_service_weights(catalog: &Catalog, n: usize) -> Vec<Vec<f64>> {
...
fn shared_capability_weights(catalog: &Catalog, n: usize) -> Vec<Vec<f64>> {
...
fn journey_services(journey: &catalog_model::journey::UseCase) -> Vec<ServiceId> {
    ...
    if let GroundedSet::Known { items } = &journey.steps {
        for step in items.iter() {
            if let Some(catalog_kernel::provenance::Grounded::Known { value, .. }) =
                &step.value.route
            {
...
fn dominant_aggregate_key(journey: &catalog_model::journey::UseCase) -> String {
...
mod tests {
    use super::*;
    use crate::journey_group::CatalogJourneyGrouping;
