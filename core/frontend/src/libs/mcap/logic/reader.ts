...
import type { ByteSource } from '../adapters/byte-source'
import { whileWaiting } from './abort'
import { createProgressGate } from './progress'
import type {
  ChunkIndexEntry, RecordingIndexPage, RecordingIndexSource,
...
function chunkFromIndexPage(entry: ChunkIndexEntry): McapChunkIndex {
  return {
...
export class McapIndexedReader {
  ...
  static async open(source: ByteSource, options: McapOpenOptions = {}): Promise<McapIndexedReader> {
    const {
      metadataOnly = false, indexSource, signal, onProgress,
    } = options
    ...
    const scanned = await McapIndexedReader.openFromRecordingIndex(source, indexSource, signal, onProgress)
  ...
  private static applyRecordingIndexPage(state: PrefixScanState, page: RecordingIndexPage): void {
    state.offset = page.offset
    state.closed = page.closed
    for (const entry of page.chunks) {
      state.chunkIndexes.push(chunkFromIndexPage(entry))
    }
    for (const entry of page.message_counts) {
      const previous = state.messageCountByChannel.get(entry.channel_id) ?? 0n
      state.messageCountByChannel.set(entry.channel_id, previous + BigInt(entry.count))
    }
    if (page.records.length > 0) {
      McapIndexedReader.applyPrefixRecords(state, parseRecords(page.records).records)
    }
  }

  private static async loadRecordingIndexPages(
    source: ByteSource,
    state: PrefixScanState,
    indexSource: RecordingIndexSource,
    from: number,
    ...
    while (true) {
      // eslint-disable-next-line no-await-in-loop
      const page = await indexSource.page(requestFrom, SERVER_INDEX_PAGE_LIMIT, signal)
      size = page.size
      McapIndexedReader.applyRecordingIndexPage(state, page)
      reportProgress()
  ...
  private static async openFromRecordingIndex(
    source: ByteSource,
    indexSource: RecordingIndexSource,
    signal?: AbortSignal,
    onProgress?: (progress: PrefixScanProgress) => void,
    ...
    const size = await McapIndexedReader.loadRecordingIndexPages(source, state, indexSource, 0, signal, onProgress)
  ...
  private static async extendFromRecordingIndex(
    source: ByteSource,
    state: PrefixScanState,
    indexSource: RecordingIndexSource,
    signal?: AbortSignal,
    ...
    const chunksBefore = state.chunkIndexes.length
    const size = await McapIndexedReader.loadRecordingIndexPages(source, state, indexSource, state.offset, signal)
