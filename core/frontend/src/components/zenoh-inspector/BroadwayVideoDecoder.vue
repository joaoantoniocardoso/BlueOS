...
export default Vue.extend({
  ...
  methods: {
    async loadBroadway() {
      if (this.broadwayLoaded) return

      try {
        this.broadwayLoaded = true
      } catch (error) {
        console.error('Error loading Broadway.js:', error)
        throw error
      }
    },

    async setupVideoDecoder() {
      if (!this.broadwayLoaded || !this.canvas) {
        return
      }
