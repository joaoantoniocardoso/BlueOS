...
| USB vehicle | `192.168.2.2` on `enp17s0f3u2u4c2` — **no stranding** (no static-IP / DHCP / interface-priority that can drop the USB path). If it loses WAN: `sshpass -p raspberry ssh pi@192.168.2.2 "sudo ip route add default via 192.168.2.1 dev eth0"` then `ping 8.8.8.8`. |
